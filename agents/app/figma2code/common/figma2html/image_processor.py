...
class ImageProcessor:
    ...
    def export_node_as_base64_png(
        ...
        if not file_key or not node_id:
            return self._create_placeholder_base64(
                node.get("width", 150), node.get("height", 150)
            )
    ...
    def process_image_node(
        ...
        if settings.get("embedImages", False):
            img_url = self.export_node_as_base64_png(
                node, has_children, file_key=fk, node_id=nid
            )
        else:
