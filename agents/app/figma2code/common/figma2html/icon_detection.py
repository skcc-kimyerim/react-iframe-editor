...
class IconDetection:
    ...
    ICON_PRIMITIVE_TYPES: Set[str] = {"RECTANGLE", "ELLIPSE", "POLYGON", "STAR", "LINE"}

    ICON_COMPLEX_VECTOR_TYPES: Set[str] = {"VECTOR", "BOOLEAN_OPERATION"}
    ...
    def is_likely_icon(self, node: Dict[str, Any], log_details: bool = False) -> bool:
        ...
        """
        info = [
            f"Node: {node.get('name', 'unnamed')} ({node.get('type', 'unknown')}, ID: {node.get('id', 'no-id')})"
        ]
        result = False
        reason = ""
        ...
        # 5. Size Check for other types
            ...
            reason = (
                f"Too large for icon: {node.get('width', 0)}x{node.get('height', 0)}"
            )
            result = False
    ...
    def _check_children_recursively(
        ...
        for child in children:
            ...
            if (
                child.get("type") in self.ICON_COMPLEX_VECTOR_TYPES
                or child.get("type") in self.ICON_PRIMITIVE_TYPES
            ):
                has_valid_content = True
            elif child.get("type") == "GROUP" and "children" in child:
                # Recursively check children of groups
                group_result = self._check_children_recursively(
                    child.get("children", [])
                )
