...
class CSSStyleBuilder:
    ...
    def add_position_styles(
        ...
        """Add position and size styles"""
        # Add size
        self._add_size_styles(node, parent_node, node.get("width", 0), node.get("height", 0))
        ...
        layout_positioning = node.get("layoutPositioning", "ABSOLUTE")
        
        if layout_positioning == "ABSOLUTE" or (parent_node and parent_node.get("layoutMode") == "NONE"):
    ...
    def _add_size_styles(self, node: Dict[str, Any], parent_node: Optional[Dict[str, Any]], width: float, height: float):
    ...
    def add_border_radius(self, node: Dict[str, Any]) -> "CSSStyleBuilder":
        ...
        radii = []
        for corner in ["topLeftRadius", "topRightRadius", "bottomRightRadius", "bottomLeftRadius"]:
        ...
        if "lineHeight" in style:
            ...
            if isinstance(line_height, dict):
                if line_height.get("unit") == "PIXELS":
                    ...
                    self.add_style("line-height", f"{line_height.get('value', 120) / 100}")
        ...
        if "letterSpacing" in style:
            ...
            if isinstance(letter_spacing, dict):
                if letter_spacing.get("unit") == "PIXELS":
                    self.add_style("letter-spacing", f"{letter_spacing.get('value', 0)}px")
    ...
    def add_auto_layout_styles(self, node: Dict[str, Any]) -> "CSSStyleBuilder":
        ...
        if layout_mode in ["HORIZONTAL", "VERTICAL"]:
            ...
            if layout_mode == "HORIZONTAL":
                self.add_style("justify-content", align_mapping.get(primary_align, "flex-start"))
                self.add_style("align-items", align_mapping.get(counter_align, "flex-start"))
            else:
                self.add_style("justify-content", align_mapping.get(primary_align, "flex-start"))
                self.add_style("align-items", align_mapping.get(counter_align, "flex-start"))
...
def build_css_for_node(
    ...
    return (builder
        .add_position_styles(node, parent_node)
