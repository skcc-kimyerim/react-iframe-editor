...
from .page_generator import PageGenerator, make_filename
...
def convert(figma_url: str, output: str, token: Optional[str]):
    """
...
@click.option(
    "--output",
    "-o",
    default="../../frontend/src/test-components",
    help="React 컴포넌트 출력 디렉토리",
)
@click.option(
    "--token", "-t", help="Figma API 토큰 (또는 FIGMA_API_TOKEN 환경변수 설정)"
)
def convert_react(figma_url: str, output: str, token: Optional[str]):
    """
...
def benchmark(figma_url: str, token: Optional[str]):
    """변환 성능 벤치마크"""
